...
# 导入后处理模块
from hapcut2_post_processing import apply_hapcut2_post_processing
        ...
        with open(vcf_file, 'r') as in_f:
            for line in in_f:
                ...
                if pos in pos_to_phasing:
                    ...
                    # 没有phasing信息，保持原样（或者可以设置PS=0表示unphased）
                    out_f.write(line)


def process_blocks(components_data, initial_haplotype, output_dir, vcf_file,
                   extractHAIRS_file=None, matrix_data=None, pos_map=None,
    ...
    - output_dir: 输出目录
    - vcf_file: 原始VCF文件路径
    - extractHAIRS_file: extractHAIRS文件路径（用于后处理）
    ...
    # 应用HapCUT2后处理（如果启用）
    if enable_post_processing and extractHAIRS_file and matrix_data and pos_map:
        print("\n" + "=" * 60)
        print("应用HapCUT2后处理以提高准确率...")
        print("=" * 60)
        ...
        try:
            # 调用后处理函数
            post_stats = apply_hapcut2_post_processing(
                vcf_file,
                extractHAIRS_file,
    ...
    if post_processing_total_time > 0:
        print(f"HapCUT2后处理总耗时: {post_processing_total_time:.4f} 秒")
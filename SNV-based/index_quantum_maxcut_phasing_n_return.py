...
def main():
    # 解析命令行参数
    if len(sys.argv) < 3:
        sys.exit('Usage: python3 %s <vcf.tab> <extractHAIRS.txt>\n'
                 'OR: python3 %s --nf <vcf.tab> <extractHAIRS.txt> <porec_vcf.tab> <porec_extractHAIRS.txt> a=<value>\n'
                 'OR: python3 %s --no-post <vcf.tab> <extractHAIRS.txt>  # 禁用后处理'
    ...
    if sys.argv[1] == '--nf':
        ...
        if len(sys.argv) != 7:
            sys.exit(
                'Usage: python3 %s --nf <vcf.tab> <extractHAIRS.txt> <porec_vcf.tab> <porec_extractHAIRS.txt> a=<value>'
                % sys.argv[0])
        ...
        print(f"检测到 --nf 参数，开始合并文件...")
        print(f"VCF文件1: {vcf1_file}")
        print(f"extractHAIRS文件1: {extractHAIRS1_file}")
        print(f"VCF文件2: {vcf2_file}")
        print(f"extractHAIRS文件2: {extractHAIRS2_file}")
        ...
        try:
            ...
            merged_hapcut2_file = os.path.join(output_dir, "merged_extractHAIRS.txt")

            # 调用合并函数，获取第一个文件的行数
            success, hapcut2_1_line_count = merge_vcf_extractHAIRS.merge_files(
                vcf1_file, extractHAIRS1_file,
                vcf2_file, extractHAIRS2_file,
                merged_vcf_file, merged_hapcut2_file
            )
            ...
            print(f"第一个hapcut2文件包含 {hapcut2_1_line_count} 行")

            # 使用合并后的文件继续处理
            vcf_file = merged_vcf_file
            extractHAIRS_file = merged_hapcut2_file
            print(f"\n使用合并后的文件继续处理...")
            print(f"合并后VCF文件: {vcf_file}")
            print(f"合并后extractHAIRS文件: {extractHAIRS_file}")
        ...
        # 正常模式，只处理单个文件对
        if len(sys.argv) != 3:
            sys.exit('Usage: python3 %s <vcf.tab> <extractHAIRS.txt>' % sys.argv[0])

        vcf_file = sys.argv[1]
        extractHAIRS_file = sys.argv[2]
        merge_time = 0  # 没有合并操作
        hapcut2_1_line_count = 0  # 单文件模式下没有第一个文件的行数
        a = 1  # 单文件模式下使用默认值，也可以改为从命令行接收
    ...
    # 如果有合并操作，添加到时间统计中
    if merge_time > 0:
        timing_results.append(("合并VCF和extractHAIRS文件 (merge_files)", merge_time))

    # 初始化碱基矩阵和正确率矩阵
    func_start = time.time()
    result = get_init_matrix.build_snp_sparse_matrix(vcf_file, extractHAIRS_file)
    ...
    func_start = time.time()
    # 传递第一个文件的行数和从命令行获取的a值
    print(f"*** 在第 {hapcut2_1_line_count + 1} 行，开始应用权重乘数 {a} ***\n")
    ...
    # 调用改进后的process_blocks函数，传入后处理所需的参数
    process_blocks(
        components_graphs,
        init_haplotype,
        phasing_output_dir,
        vcf_file,
        extractHAIRS_file=extractHAIRS_file,  # 传入extractHAIRS文件
        matrix_data=matrix,  # 传入矩阵数据
        pos_map=pos_map,  # 传入位置映射
        enable_post_processing=enable_post_processing  # 是否启用后处理
    )
    ...
    if enable_post_processing:
        timing_results.append(("处理块并生成phased VCF (含HapCUT2后处理)", process_blocks_time))
    else:
    ...
    with open(run_time_file, 'w') as f:
        ...
        if '--nf' in sys.argv:
            ...
            f.write(f"  extractHAIRS文件1: {sys.argv[3]}\n")
            f.write(f"  VCF文件2: {sys.argv[4]}\n")
            f.write(f"  extractHAIRS文件2: {sys.argv[5]}\n")
            f.write(f"  a值: {a}\n")  # 记录a值
            f.write(f"  合并后VCF文件: {vcf_file}\n")
            f.write(f"  合并后extractHAIRS文件: {extractHAIRS_file}\n")
            f.write(f"  第一个extractHAIRS文件行数: {hapcut2_1_line_count}\n")
        else:
            f.write("输入文件（单文件模式）:\n")
            f.write(f"  VCF文件: {vcf_file}\n")
            f.write(f"  extractHAIRS文件: {extractHAIRS_file}\n")
            f.write(f"  a值: {a}\n")  # 记录a值

        f.write(f"  HapCUT2后处理: {'启用' if enable_post_processing else '禁用'}\n")
        f.write("-" * 60 + "\n")
        f.write("输出文件:\n")
    ...
    # 清理临时文件（如果是合并模式，可以选择是否删除中间文件）
    if '--nf' in sys.argv:
        print(f"\n注意：合并后的中间文件已保存在:")
        print(f"  - {vcf_file}")
        print(f"  - {extractHAIRS_file}")

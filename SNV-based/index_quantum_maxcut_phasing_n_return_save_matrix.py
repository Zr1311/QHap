...
# 新增：保存碱基矩阵到txt文件（映射回实际position和fragment）
def save_base_matrix_to_txt(sparse_matrix, index_to_fragment, column_to_position, output_path):
    """
    将碱基矩阵保存为txt文件，包含实际的fragment名称、基因组位置和碱基类型
    """
    with open(output_path, 'w', encoding='utf-8') as f:
        # 写入表头
        f.write("fragment\tposition\tbase\n")
        # 遍历稀疏矩阵，映射并写入数据
        for row_idx, col_idx, base in sparse_matrix:
            fragment_name = index_to_fragment.get(row_idx, f"fragment_{row_idx}")
            actual_position = column_to_position.get(col_idx, f"pos_{col_idx}")
            f.write(f"{fragment_name}\t{actual_position}\t{base}\n")

    print(f"\n碱基矩阵已保存到txt文件: {output_path}")
    print(f"矩阵包含 {len(sparse_matrix)} 条有效碱基记录")
...
def process_phasing(vcf_file, extractHAIRS_file, overall_start_time=None):
    ...
    # 获取反向映射（用于保存实际名称和位置）
    index_to_fragment = result['index_to_fragment']
    column_to_position = result['column_to_position']
    function_times["build_snp_sparse_matrix"] = matrix_time

    # 保存碱基矩阵到txt文件（映射回实际fragment和position）
    base_matrix_txt_path = os.path.join(matrix_file_dir, "base_matrix_with_mapping.txt")
    save_base_matrix_to_txt(matrix, index_to_fragment, column_to_position, base_matrix_txt_path)

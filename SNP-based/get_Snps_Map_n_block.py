...
def process_chunk_optimized(chunk_data: List[Tuple[int, int, float]], col_indices: List[int],
                            fragment_reads_1_line_count: int, a: float) -> Dict[Tuple[int, int], float]:
    ...
    for row_idx, col_value_pairs in row_data.items():
        ...
        for idx1 in range(num_nz):
            ...
            for idx2 in range(idx1 + 1, end_idx):
                ...
                if 0 < fragment_reads_1_line_count < row_idx + 1:
...
Optional[Set[int]]]:
    ...
    print(f"{len(matrix_data)} non-zero records after filtering")
    ...
    print(f"Split data into {len(chunks)} chunks with ~{optimal_chunk_size} records each")
    ...
    print(f"Calculated {len(total_weights)} non-zero weights between column pairs")
...
def find_connected_components_ultra_optimized(edge_weights: Dict[Tuple[int, int], float],
    ...
    for component in components_dict.values():
        ...
        print(f"Component {len(components_data)}: {len(component)} nodes, {len(component_edges)} edges")

    print(f"Found {len(components_dict)} total components, {single_node_components} single-node components filtered")
    print(f"Retained {len(components_data)} valid components (>=2 nodes)")
...
def find_connected_components_bfs_optimized(edge_weights: Dict[Tuple[int, int], float],
    ...
    for start_idx in range(n):
        ...
        for idx1 in component_indices:
            ...
            for idx2 in adjacency[idx1]:
                node2 = nodes[idx2]
                if node1 < node2 and node2 in component:
    ...
    print(f"Found {n - sum(visited) + len(components_data) + single_node_components} total components, "
          f"{single_node_components} single-node components filtered")
    print(f"Retained {len(components_data)} valid components (>=2 nodes)")
...
def main_optimized(matrix_weight: List[Tuple[int, int, float]],
    ...
    if edge_weights is None:
        print("Edge weight calculation failed")
        return None

    compute_weights_end_time = time.time()
    compute_weights_time = compute_weights_end_time - compute_weights_start_time
    print(f"Edge weight calculation time: {compute_weights_time:.2f}s")

    find_components_start_time = time.time()

    if use_union_find:
        print("Finding connected components using Union-Find...")
    ...
    print(f"Component detection time: {find_components_time:.2f}s")
    ...
    if return_networkx:
        ...
        print(f"NetworkX conversion time: {convert_time:.2f}s")
...
def main(matrix_weight, pos_to_col_idx, return_networkx=False, use_union_find=False, min_component_size=2,
         fragment_reads_1_line_count=0, a=1.0):
...
def compute_edge_weights(matrix_weight, pos_to_col_idx, fragment_reads_1_line_count=0, a=1.0):
    return compute_edge_weights_optimized(matrix_weight, pos_to_col_idx, fragment_reads_1_line_count, a)
...
def process_chunk(chunk_data, col_indices, fragment_reads_1_line_count=0, a=1.0):
    return process_chunk_optimized(chunk_data, col_indices, fragment_reads_1_line_count, a)